...
export const api = {
  listMedicines: async (): Promise<Medicine[]> =>
    (await fetch(`${API}/medicines`)).json(),
  ...
  listPharmacies: async (): Promise<Pharmacy[]> =>
    (await fetch(`${API}/pharmacies`)).json(),
  ...
  listDistributions: async (): Promise<Distribution[]> =>
    (await fetch(`${API}/distributions`)).json(),
  ...
  getKPI: async (
    ...
    totalSKUs: number;
    currency: 'UZS' | 'USD';
  }> => (await fetch(`${API}/kpi${month ? `?month=${month}` : ''}`)).json(),
